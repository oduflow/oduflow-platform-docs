...
ODUFLOW_GITHUB_CLIENT_TOKEN=<client repository token>
...
`ODUFLOW_GITHUB_CLIENT_TOKEN` is unnecessary when the client already has an
explicit encrypted Git credential bundle. LiteLLM and provider credentials are
